#![warn(missing_docs)]
//!
//! This crate provides a library for working with [Passage](https://passage.id),
//! a modern passwordless authentication experience based on passkeys and magic links.
//! This library currently implements a very small subset of the available
//! Passage API endpoints.
//!
//! See Passage [Authentication API](https://docs.passage.id/api-docs/authentication-api)
//! and [Management API](https://docs.passage.id/api-docs/management-api)
//! for all the possible endpoints that could be added to this crate in the future.
//!
//! ## Usage
//!
...
/// Passage is the main entry point you'll be working with.
/// See [Passage::new] for details on where to find these values for your app.
pub struct Passage {
    app_id: String,
    #[allow(dead_code)]
    api_key: String,
...
impl Passage {
    ...
    /// // Find your app id at https://console.passage.id/settings
    /// let app_id = "cHxJnV5eqc8aIrgQjgfIEsMl".to_string();
    ///
    /// // Create an api key for your app at
    /// // https://console.passage.id/settings/apikeys
    /// let api_key = "P5H1fLoYIv.MxVnHArwp8xJ3q0FDoByoUiDJC0r5ABcqK6ozXyFmeacdNfySuaFLYN75YrS6JT1".to_string();
    ///
    /// let passage = Passage::new(app_id, api_key);
    /// ```
    ...
    /// Updates a [Passage] to set a static public key.
    ///
    /// This is currently the only way to provide a public key,
    /// but in the future, the library may look up the key dynamically
    /// using your `app_id` if it's not provided here.
    ///
    /// ## Example
    ///
    /// ```rust
    ...
    /// #
    /// // Download the public jwk key for your app from
    /// // https://auth.passage.id/v1/apps/{app_id}/.well-known/jwks.json.
    /// // You only want the key itself, not the array.
    /// let pub_key = r#"{
    ///     "alg": "RS256",
    ///     "kty": "RSA",
    ///     "use": "sig",
    ...
    ///   }"#.to_string();
    ///
    /// let passage = Passage::new(app_id, api_key).set_pub_jwk(pub_key);
    /// ```
    ...
    /// When successful, the resulting `String` is the authenticated Passage user ID.
    /// See [Validation Passage JWTs](https://docs.passage.id/backend/overview/other#validation-passage-jwts) for details.
    ///
    /// ## Example
    ///
    /// ```rust
    ...
    /// // [Passage Element](https://docs.passage.id/frontend/passage-element),
    /// // the Passage authentication JWT will be sent to your application
    /// // via a cookie with the key `psg_auth_token`
    /// let psg_auth_token = "...";
    ///
    ...
    pub fn authenticate_token(&self, token: &str) -> Result<String, AuthError> {
        let pub_jwk = self.pub_jwk.as_ref().ok_or(AuthError::PubKeyMissing)?;
...
mod tests {
    ...
    #[test]
    fn init_passage() {
        let passage = Passage::new(String::from("test"), String::from("fake_api_key"));

        assert_eq!(passage.app_id, String::from("test"));
        assert!(passage.pub_jwk.is_none());
    }

    #[test]
    fn set_pub_jwk() {
        let passage = Passage::new(String::from("test"), String::from("fake_api_key"))
            .set_pub_jwk(String::from(PUB_JWK));
    ...
    fn authenticate_good_token() {
        ...
        let passage = Passage::new(String::from("fake"), String::from("fake_api_key"))
            .set_pub_jwk(String::from(PUB_JWK));
    ...
    fn reject_missing_pub_key() {
        ...
        match res {
            Err(AuthError::PubKeyMissing) => assert!(true),
            _ => assert!(
                false,
                "missing pub key was not properly rejected: {:?}",
                res
            ),
        }
    }
